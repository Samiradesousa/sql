SELECT * FROM cliente WHERE cliente.clienteId IN (
    SELECT DISTINCT p.funcionarioId
    FROM proposta p
    INNER JOIN funcionarioBons f ON p.funcionarioId = f.funcionarioId
);
...
SELECT f.nomeFuncionario, c.nomeCliente, p.dataInicio, p.dataConclusao FROM funcionario f
